...
//
// Modified by Paul Gao to support consuming multiple tokens in one cycle
//
...
module bsg_async_credit_counter_serdes #(parameter max_tokens_p = "inv"
    ...
    , input r_clk_i
    , input r_reset_i
	// support consuming multiple tokens in one cycle
    , input [lg_credit_to_token_decimation_p+1-1:0] r_dec_credit_i 
    , input r_infinite_credits_i  // basically suppress this module
    , output r_credits_avail_o
	// Output number of remaining tokens in this counter
	, output logic [lg_credit_to_token_decimation_p+1-1:0] r_credits_avail_num_o
    );
...
*/
   ...
   always_comb begin
		r_credits_avail_num_o = (1 << lg_credit_to_token_decimation_p) - r_counter_r[0+:lg_credit_to_token_decimation_p];
   end
