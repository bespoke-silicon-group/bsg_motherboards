...
#define IIC_PLL_BASE_ADDRESS    XPAR_AXI_IIC_DIG_POT_PLL_BASEADDR
...
static int write_to_digpot_pll(u8* write_buffer, u16 byte_count)
{
    u16 sent_count;
    u32 status_reg;

    // wait IIC to be idle
    while (((status_reg = XIic_ReadReg(IIC_PLL_BASE_ADDRESS, XIIC_SR_REG_OFFSET)) &
        (XIIC_SR_RX_FIFO_EMPTY_MASK | XIIC_SR_TX_FIFO_EMPTY_MASK | XIIC_SR_BUS_BUSY_MASK))
        != (XIIC_SR_RX_FIFO_EMPTY_MASK | XIIC_SR_TX_FIFO_EMPTY_MASK));
    ...
    sent_count = 0;
    while (sent_count == 0) {
        status_reg = XIic_ReadReg(IIC_PLL_BASE_ADDRESS, XIIC_SR_REG_OFFSET);
        if (!(status_reg & XIIC_SR_BUS_BUSY_MASK)) {
            sent_count = XIic_DynSend(IIC_PLL_BASE_ADDRESS, IIC_DIG_POT_PLL_SLAVE_ADDRESS, write_buffer, byte_count, XIIC_STOP);
        }
    }
...
static int read_from_digpot_pll(u8 command, u8 parameter, u8* read_buffer, u16 byte_count)
{
    u16 sent_count;
    u16 recv_count;
    u32 status_reg;
    ...
    while (sent_count == 0) {
        status_reg = XIic_ReadReg(IIC_PLL_BASE_ADDRESS, XIIC_SR_REG_OFFSET);
        if (!(status_reg & XIIC_SR_BUS_BUSY_MASK)) {
            sent_count = XIic_DynSend(IIC_PLL_BASE_ADDRESS, IIC_DIG_POT_PLL_SLAVE_ADDRESS, send_buffer, 2, XIIC_REPEATED_START);
        }
    }
    if (sent_count != 2) return XST_FAILURE;

    // start receiving
    recv_count = XIic_DynRecv(IIC_PLL_BASE_ADDRESS, IIC_DIG_POT_PLL_SLAVE_ADDRESS, read_buffer, byte_count);
...
int dig_pot_init()
{
	...
	status = XIic_DynInit(IIC_PLL_BASE_ADDRESS);
    if (status != XST_SUCCESS) return XST_FAILURE;
    while (((status_reg = XIic_ReadReg(IIC_PLL_BASE_ADDRESS, XIIC_SR_REG_OFFSET)) &
        (XIIC_SR_RX_FIFO_EMPTY_MASK | XIIC_SR_TX_FIFO_EMPTY_MASK | XIIC_SR_BUS_BUSY_MASK))
        != (XIIC_SR_RX_FIFO_EMPTY_MASK | XIIC_SR_TX_FIFO_EMPTY_MASK));
    ...
    write_buffer[0] = 0xD0;
    write_buffer[1] = 0x04;
    status = write_to_digpot_pll(write_buffer, 2);
    if (status != XST_SUCCESS) return XST_FAILURE;
...
int dig_pot_prog_eeprom_default()
{
    ...
    write_buffer[0] = 0xD0;
    write_buffer[1] = 0x07;
    status = write_to_digpot_pll(write_buffer, sizeof(write_buffer));
    if (status != XST_SUCCESS) return XST_FAILURE;

    // Program eeprom for PLL
    write_buffer[0] = 0x80;
    write_buffer[1] = PLL_DEFAULT_RES;
    status = write_to_digpot_pll(write_buffer, sizeof(write_buffer));
    if (status != XST_SUCCESS) return XST_FAILURE;
	
	// Program eeprom for LDO
    write_buffer[0] = 0x82;
    write_buffer[1] = LDO_DEFAULT_RES;
    status = write_to_digpot_pll(write_buffer, sizeof(write_buffer));
    if (status != XST_SUCCESS) return XST_FAILURE;
...
int dig_pot_prog_eeprom_voltage_now()
{
	...
	// Real current PLL voltage
	status = dig_pot_report_pll(&PLL_voltage_now);
	if (status != XST_SUCCESS) return XST_FAILURE;
	
	// Real current LDO voltage
	status = dig_pot_report_ldo(&LDO_voltage_now);
	if (status != XST_SUCCESS) return XST_FAILURE;
    ...
    // Program eeprom for PLL
    write_buffer[0] = 0x80;
    write_buffer[1] = PLL_voltage_now;
    status = write_to_digpot_pll(write_buffer, sizeof(write_buffer));
    if (status != XST_SUCCESS) return XST_FAILURE;
	
	// Program eeprom for LDO
    write_buffer[0] = 0x82;
    write_buffer[1] = LDO_voltage_now;
    status = write_to_digpot_pll(write_buffer, sizeof(write_buffer));
    if (status != XST_SUCCESS) return XST_FAILURE;
...
int dig_pot_update_pll(u8 val)
{
    int status;
    u8 read_back_val;
    if (val > PLL_MAX_RES) return XST_FAILURE;
    write_buffer[0] = 0x10;
    write_buffer[1] = val;
    status = write_to_digpot_pll(write_buffer, 2);
    if (status != XST_SUCCESS) return XST_FAILURE;
    status = dig_pot_report_pll(&read_back_val);
    if (status != XST_SUCCESS) return XST_FAILURE;
    if (read_back_val != val) return XST_FAILURE;
    return XST_SUCCESS;
}
...
{
    int status;
    status = read_from_digpot_pll(0x30, 0x03, rtn, 1);
    if (status != XST_SUCCESS) return XST_FAILURE;
    return XST_SUCCESS;
}
...
int dig_pot_update_ldo(u8 val)
{
    int status;
    u8 read_back_val;
    if (val > LDO_MAX_RES) return XST_FAILURE;
    write_buffer[0] = 0x11;
    write_buffer[1] = val;
    status = write_to_digpot_pll(write_buffer, 2);
    if (status != XST_SUCCESS) return XST_FAILURE;
    status = dig_pot_report_ldo(&read_back_val);
    if (status != XST_SUCCESS) return XST_FAILURE;
    if (read_back_val != val) return XST_FAILURE;
    return XST_SUCCESS;
}
...
{
    int status;
    status = read_from_digpot_pll(0x31, 0x03, rtn, 1);
    if (status != XST_SUCCESS) return XST_FAILURE;
    return XST_SUCCESS;
}
