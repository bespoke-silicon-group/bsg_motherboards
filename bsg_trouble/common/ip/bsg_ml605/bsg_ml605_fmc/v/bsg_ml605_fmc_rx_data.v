...
//------------------------------------------------------------
// File: bsg_ml605_fmc_rx_data.v
//
// Authors: Luis Vega - lvgutierrez@eng.ucsd.edu
//------------------------------------------------------------

module bsg_ml605_fmc_rx_data #
  ...
  generate
    for (i = 0; i < 11; i++) begin
      ...
      iserdes_master_data_inst
        ...
        ,.SHIFTOUT1(iserdes_shiftout_1_lo[i])
        ,.SHIFTOUT2(iserdes_shiftout_2_lo[i])
      ...
      ISERDESE1 #
        ...
        ,.SRVAL_Q3(1'b0)
        ,.SRVAL_Q4(1'b0))
      iserdes_slave_data_inst
        (.O()
        ,.Q1()
        ,.Q2()
        ,.Q3(iserdes_data_lo[(8*i) + 1])
        ,.Q4(iserdes_data_lo[(8*i) + 0])
        ,.Q5()
        ,.Q6()
        ,.SHIFTOUT1()
        ,.SHIFTOUT2()
        ...
        ,.OFB(1'b0)
        ,.RST(reset_i)
        ,.SHIFTIN1(iserdes_shiftout_1_lo[i])
        ,.SHIFTIN2(iserdes_shiftout_2_lo[i]));

      bsg_ml605_fmc_rx_data_bitslip_ctrl bitslip_ctrl_inst
        (.clk_i(clk_div_i)
        ,.reset_i(reset_i)
