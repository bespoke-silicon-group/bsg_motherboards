...
# Test mode
if {![info exists env(bsg_dcdc)] && ![info exists env(bsg_mitm)]} {
  error "ERROR(BSG): One test mode (dcdc/mitm) should be defined"
} elseif {[info exists env(bsg_dcdc)] && [info exists env(bsg_mitm)]} {
  error "ERROR(BSG): Only one test mode (dcdc/mitm) can be defined"
} elseif {[info exists env(bsg_dcdc)] && ![info exists env(bsg_mitm)]} {
  set_option -hdl_define -set "bsg_dcdc"
} elseif {![info exists env(bsg_dcdc)] && [info exists env(bsg_mitm)]} {
  set_option -hdl_define -set "bsg_mitm"
}
