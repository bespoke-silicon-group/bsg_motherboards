...
// * [09-26-2017] Paul Gao modified to support SERDES
//
// clocks:
//
...
module bsg_comm_link_serdes #
  (parameter channel_width_p = "inv"
  ,parameter core_channels_p = "inv"
  ,parameter link_channels_p = "inv"
  ,parameter serdes_ratio_p = "inv"
  ,parameter channel_width_serdes_p = channel_width_p*serdes_ratio_p+serdes_ratio_p+$clog2(serdes_ratio_p+1)
  ...
  ,input  [link_channels_p-1:0] io_valid_0_tline_i
  ,input  [channel_width_p-1:0] io_data_0_tline_i [link_channels_p-1:0]
  ,input  [link_channels_p-1:0] io_valid_1_tline_i
  ,input  [channel_width_p-1:0] io_data_1_tline_i [link_channels_p-1:0]
  ,output [link_channels_p-1:0] io_token_clk_tline_o

  // im out
  ,output [link_channels_p-1:0] im_clk_tline_o
  ,output [serdes_ratio_p-1:0] im_valid_tline_o [link_channels_p-1:0]
  ,output [channel_width_serdes_p-serdes_ratio_p-$clog2(serdes_ratio_p+1)-1:0] im_data_tline_o [link_channels_p-1:0]
  ...
  logic                       fast_core_kernel_calib_done_lo;
  logic                       im_kernel_calib_done_lo;
  logic [link_channels_p-1:0] core_kernel_active_channels_lo;

  logic [link_channels_p*2-1:0] core_kernel_valid_lo;
  logic [channel_width_p-1:0] core_kernel_data_lo [link_channels_p*2-1:0];
  logic [link_channels_p*2-1:0] core_fuser_yumi_lo;

  logic [link_channels_p*2-1:0] core_kernel_valid_pre_lo;
  logic [channel_width_p-1:0] core_kernel_data_pre_lo [link_channels_p*2-1:0];
  logic [link_channels_p*2-1:0] core_fuser_ready_pre_lo, core_fuser_yumi_pre_lo;

  logic [link_channels_p-1:0] core_fuser_valid_lo;
  logic [channel_width_serdes_p-1:0] core_fuser_data_lo [link_channels_p-1:0];
  logic [link_channels_p-1:0] core_kernel_ready_lo;

  bsg_comm_link_fuser_serdes #
    (.channel_width_p(channel_width_p)
	,.channel_width_serdes_p(channel_width_serdes_p)
  ...
  fuser
	...
	,.im_reset_i(~im_kernel_calib_done_lo)
    // ctrl
    ,.core_calib_done_r_i(core_kernel_calib_done_lo)
	,.fast_core_clk_i(fast_core_clk_i)
    ,.fast_reset_i(~fast_core_kernel_calib_done_lo)
	,.fast_core_calib_done_r_i(fast_core_kernel_calib_done_lo)
  ...
  kernel
	...
	,.fast_core_calib_done_r_o(fast_core_kernel_calib_done_lo)
	,.im_calib_done_r_o(im_kernel_calib_done_lo)
    ...
    ,.io_valid_0_tline_i(io_valid_0_tline_i)
    ,.io_data_0_tline_i(io_data_0_tline_i)
	,.io_valid_1_tline_i(io_valid_1_tline_i)
    ,.io_data_1_tline_i(io_data_1_tline_i)
