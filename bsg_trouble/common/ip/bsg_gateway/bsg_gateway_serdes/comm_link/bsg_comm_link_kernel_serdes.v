...
module bsg_comm_link_kernel_serdes #
  ...
  ,output                       fast_core_calib_done_r_o
  ,output                       im_calib_done_r_o
  ,output [link_channels_p-1:0] core_active_channels_o
  ,output                       core_async_reset_danger_o

  // core in
  ,input  [link_channels_p-1:0] core_valid_i
  ,input  [channel_width_serdes_p-1:0] core_data_i [link_channels_p-1:0]
  ,output [link_channels_p-1:0] core_ready_o

  // core out
  ,output [link_channels_p*2-1:0] core_valid_o
  ,output [channel_width_p-1:0] core_data_o [link_channels_p*2-1:0]
  ,input  [link_channels_p*2-1:0] core_yumi_i
...
//  logic [link_channels_p-1:0] io_infinite_credits_en;
  ...
  logic fast_core_channel_reset_lo;
  ...
  if (master_p) begin : mstr
   ...
   logic  im_calib_done_lo;
    ...
    bssi_reset
      ...
      ,.iclk_reset_i(1'b0)
      ,.oclk_i(fast_core_clk_i)
      ,.iclk_data_i(im_channel_reset_lo)
      ,.iclk_data_o()
      ,.oclk_data_o(fast_core_channel_reset_lo));
    ...
    for (i=0; i < link_channels_p; i=i+1) begin: ch
      ...
      assign core_loopback_en[i] = 1'b0;
       ...
       bsg_launch_sync_sync #
         (.width_p(1))
       out_to_fast_core_sync_calib_done
         (.iclk_i(io_master_clk_i)
          ,.iclk_reset_i(1'b0)
          ,.oclk_i(fast_core_clk_i)
          ,.iclk_data_i(im_calib_done_lo)
          ,.iclk_data_o()
          ,.oclk_data_o(fast_core_calib_done_r));
...
// synopsys translate_on
	   ...
	   assign fast_core_calib_done_r_o = fast_core_calib_done_r;
	   assign im_calib_done_r_o = im_calib_done_r;
  ...
  for (i=0; i < link_channels_p; i=i+1) begin: ch
    ...
    sso
      // core clk and reset
      (.core_clk_i(io_master_clk_i)
      ,.core_reset_i(im_channel_reset_lo)
	  
      ,.core_valid_i(core_valid_i[i])
      ,.core_data_i(core_data_i[i])
    ...
    ssi
      ...
      ,.io_data_0_i(io_data_0_tline_i[i])
      ,.io_valid_0_i(io_valid_0_tline_i[i])
	  ,.io_data_1_i(io_data_1_tline_i[i])
      ,.io_valid_1_i(io_valid_1_tline_i[i])
      ...
      // core clk and reset
      ,.core_clk_i(fast_core_clk_i)
      ,.core_reset_i(fast_core_channel_reset_lo)

      // core out
      ,.core_valid_0_o(core_valid_o[i])
      ,.core_data_0_o(core_data_o[i])
      ,.core_yumi_0_i(core_yumi_i[i])
	  
	  ,.core_valid_1_o(core_valid_o[i+link_channels_p])
      ,.core_data_1_o(core_data_o[i+link_channels_p])
      ,.core_yumi_1_i(core_yumi_i[i+link_channels_p])
       );
