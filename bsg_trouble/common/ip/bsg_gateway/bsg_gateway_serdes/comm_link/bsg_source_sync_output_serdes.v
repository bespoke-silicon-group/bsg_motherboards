...
//
// Modified by Paul Gao to support SERDES output
//
// this implements:
...
//
module bsg_source_sync_output_serdes
     #(
       parameter   lg_start_credits_p              = 5
       , parameter lg_credit_to_token_decimation_p = 3 // 8:1
	   , parameter channel_width_p                 = 8
       , parameter channel_width_serdes_p          = 48
	   , parameter serdes_ratio_p                  = 5
       ...
       , parameter inactive_pattern_p = {(channel_width_serdes_p-serdes_ratio_p-$clog2(serdes_ratio_p+1)){2'b01}}
       )
   (
    // going out of core; uses core clock
    input                           core_clk_i
    , input                         core_reset_i
    , input [channel_width_serdes_p-1:0]   core_data_i
    , input                         core_valid_i
    ...
    , output logic  [channel_width_serdes_p-serdes_ratio_p-$clog2(serdes_ratio_p+1)-1:0] io_data_r_o  // sdo_data
    , output logic  [serdes_ratio_p-1:0]                 io_valid_r_o // sdo_valid
   ...
   wire [channel_width_serdes_p-1:0] core_twofer_data;

   bsg_two_fifo #(.width_p(channel_width_serdes_p)) twofer
     (.clk_i(core_clk_i)
   ...
   logic [channel_width_serdes_p-1:0] io_data_n;
   logic                       io_valid_n;
   ...
   always @(posedge io_master_clk_i)
     begin
        ...
        if (io_override_en_i)
          { io_valid_r_o[0], io_data_r_o[0+:channel_width_p] }   <= io_override_valid_data_i;
        else
          begin
             if (io_valid_n) begin
			   io_valid_r_o <= io_data_n[(channel_width_serdes_p-serdes_ratio_p-$clog2(serdes_ratio_p+1))+:serdes_ratio_p];
               io_data_r_o <= io_data_n[0+:(channel_width_serdes_p-serdes_ratio_p-$clog2(serdes_ratio_p+1))];
             end else begin
			   io_valid_r_o <= 0;
               //io_data_r_o <= inactive_pattern_p;
			   io_data_r_o <= io_data_n[0+:(channel_width_serdes_p-serdes_ratio_p-$clog2(serdes_ratio_p+1))];
			 end
          end
     end
   ...
   bsg_async_fifo #(.lg_size_p(3)
                    ,.width_p(channel_width_serdes_p)
                    ) baf
   (
    .w_clk_i(core_clk_i)
   ...
   // Special token arrangement for SERDES
   logic [lg_credit_to_token_decimation_p+1-1:0] token_deque_size;
   always_comb begin
		token_deque_size = 0 + io_data_n[(channel_width_serdes_p-$clog2(serdes_ratio_p+1))+:$clog2(serdes_ratio_p+1)];
   end
   ...
   always @(posedge io_master_clk_i)
     begin
        if (io_reset_i)
          ...
          if (io_valid_n)
            token_alternator_r <= token_alternator_r + token_deque_size;
     end

   // high bit set means we have exceeded number of posedge credits
   // and are doing negedge credits
   logic [lg_credit_to_token_decimation_p+1-1:0] negedge_token_deque_size, posedge_token_deque_size;
   logic first_edge_token = token_alternator_r[lg_credit_to_token_decimation_p];
   logic io_negedge_credits_avail, io_posedge_credits_avail;
   logic [lg_credit_to_token_decimation_p+1-1:0] io_negedge_credits_avail_num, io_posedge_credits_avail_num;
   
   // Consuming tokens from correct token counters
   // It will first look at the top counter
   // If token not enough, it will look at another one
   always_comb
   begin
		negedge_token_deque_size = 0;
		posedge_token_deque_size = 0;
		io_valid_n = 0;
		if (io_data_avail == 1'b1) begin
			if (first_edge_token == 1'b0 && io_posedge_credits_avail == 1'b1) begin
				if (token_deque_size <= io_posedge_credits_avail_num) begin
					posedge_token_deque_size = token_deque_size;
					io_valid_n = 1;
				end else if (io_negedge_credits_avail == 1'b1) begin
					posedge_token_deque_size = io_posedge_credits_avail_num;
					negedge_token_deque_size = token_deque_size - io_posedge_credits_avail_num;
					io_valid_n = 1;
				end
			end
			if (first_edge_token == 1'b1 && io_negedge_credits_avail == 1'b1) begin
				if (token_deque_size <= io_negedge_credits_avail_num) begin
					negedge_token_deque_size = token_deque_size;
					io_valid_n = 1;
				end else if (io_posedge_credits_avail == 1'b1) begin
					negedge_token_deque_size = io_negedge_credits_avail_num;
					posedge_token_deque_size = token_deque_size - io_negedge_credits_avail_num;
					io_valid_n = 1;
				end
			end
		end
   ...
   bsg_async_credit_counter_serdes
	...
	,.w_inc_token_i(1'b1)
        ...
        ,.r_clk_i  (io_master_clk_i)
        ,.r_reset_i(io_reset_i     )
        ,.r_dec_credit_i      (posedge_token_deque_size)
        ,.r_infinite_credits_i(io_infinite_credits_i)
        ,.r_credits_avail_o   (io_posedge_credits_avail)
		,.r_credits_avail_num_o(io_posedge_credits_avail_num)
        );
   ...
   bsg_async_credit_counter_serdes
	...
	,.w_inc_token_i(1'b1)
        ...
        ,.r_clk_i             (io_master_clk_i         )
        ,.r_reset_i           (io_reset_i              )
        ,.r_dec_credit_i      (negedge_token_deque_size)
        ,.r_infinite_credits_i(io_infinite_credits_i)
        ,.r_credits_avail_o(io_negedge_credits_avail)
		,.r_credits_avail_num_o(io_negedge_credits_avail_num)
        );
