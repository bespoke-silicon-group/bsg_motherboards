...
//
// Modified by Paul Gao to support IDDR2 input
//
// this implements:
...
module bsg_source_sync_input_serdes #(parameter lg_fifo_depth_p=5
   ...
   (
    ...
    , input  [channel_width_p-1:0] io_data_0_i    // sdi_data
    , input                        io_valid_0_i   // sdi_valid
	, input  [channel_width_p-1:0] io_data_1_i    // sdi_data
    , input                        io_valid_1_i   // sdi_valid
    ...
    , output [channel_width_p-1:0] core_data_0_o
    , output                       core_valid_0_o
    , input                        core_yumi_0_i   
	, output [channel_width_p-1:0] core_data_1_o
    , output                       core_valid_1_o
    , input                        core_yumi_1_i   
   ...
   logic core_deq_0, core_deq_1;
   logic [channel_width_p-1:0] io_data_negedge_r;

   // capture IDDR2 data and valid
   always @(posedge io_clk_i)
     begin
        io_data_0_r  <= io_data_0_i;    io_valid_0_r <= io_valid_0_i;
        io_data_1_r  <= io_data_1_i;    io_valid_1_r <= io_valid_1_i;
     end
...
*/
   // synopsys translate_on
	...
	logic io_toggle_r, io_toggle_n;
	logic io_valid_0_lo_r, io_valid_1_lo_r;
	logic io_valid_0_lo_n, io_valid_1_lo_n;
	logic [channel_width_p-1:0] io_data_0_lo_r, io_data_1_lo_r;
	logic [channel_width_p-1:0] io_data_0_lo_n, io_data_1_lo_n;	
	...
	always @(posedge io_clk_i) begin
		if (io_reset_i) begin
			...
			io_toggle_r <= io_toggle_n;
			io_valid_0_lo_r <= io_valid_0_lo_n;
			io_valid_1_lo_r <= io_valid_1_lo_n;
			io_data_0_lo_r <= io_data_0_lo_n;
			io_data_1_lo_r <= io_data_1_lo_n;
		end
	end
	
	// Decide whether to swap the input channels from IDDR2
	// to ensure the sequence of input data to fifos
	always_comb begin
		io_toggle_n = io_toggle_r;
		io_valid_0_lo_n = io_valid_0_r;
		io_valid_1_lo_n = io_valid_1_r;
		io_data_0_lo_n = io_data_0_r;
		io_data_1_lo_n = io_data_1_r;
		if (io_valid_0_r^io_valid_1_r) begin
			io_toggle_n = ~io_toggle_r;
		end
		if (((~io_toggle_r)&(~io_valid_0_r)&io_valid_1_r)|(io_toggle_r&io_valid_0_r)) begin
			io_valid_0_lo_n = io_valid_1_r;
			io_valid_1_lo_n = io_valid_0_r;
			io_data_0_lo_n = io_data_1_r;
			io_data_1_lo_n = io_data_0_r;
		end
	end
	
	bsg_async_fifo #
	(.lg_size_p(lg_fifo_depth_p-1)  // 16 elements
	,.width_p(channel_width_p)) 
	baf_0
	(.w_clk_i(io_clk_i)
	,.w_reset_i(io_reset_i)
	,.w_enq_i(io_valid_0_lo_r)
	,.w_data_i(io_data_0_lo_r)
	,.w_full_o()
	,.r_clk_i(core_clk_i)
	,.r_reset_i(core_reset_i)
	,.r_deq_i(core_deq_0&core_valid_0)
	,.r_data_o(core_data_0)
	,.r_valid_o(core_valid_0)
	);
	...
	(.w_clk_i(io_clk_i)
	,.w_reset_i(io_reset_i)
	,.w_enq_i(io_valid_1_lo_r)
	,.w_data_i(io_data_1_lo_r)
	,.w_full_o()
	,.r_clk_i(core_clk_i)
	,.r_reset_i(core_reset_i)
	,.r_deq_i(core_deq_1&core_valid_1)
	,.r_data_o(core_data_1)
	,.r_valid_o(core_valid_1)
	);
	...
	,.ready_o(core_deq_0)
	,.data_i(core_data_0)
	,.v_i(core_valid_0&core_deq_0)
	,.v_o(core_valid_0_o)
	,.data_o(core_data_0_o)
	,.yumi_i(core_yumi_0_i)
	);	
	...
	,.ready_o(core_deq_1)
	,.data_i(core_data_1)
	,.v_i(core_valid_1&core_deq_1)
	,.v_o(core_valid_1_o)
	,.data_o(core_data_1_o)
	,.yumi_i(core_yumi_1_i)
	);	
...
//
   ...
   bsg_async_ptr_gray_serdes #(.lg_size_p(lg_fifo_depth_p+1)) bapg
   (.w_clk_i   (core_clk_i)
    ,.w_reset_i(core_reset_i)
    ,.w_inc_0_i(core_valid_0&core_deq_0)
	,.w_inc_1_i(core_valid_1&core_deq_1)
    ,.r_clk_i  (io_clk_i)
